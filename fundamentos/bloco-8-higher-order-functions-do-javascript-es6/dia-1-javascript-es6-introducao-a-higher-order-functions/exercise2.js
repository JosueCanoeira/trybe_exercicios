/*2 - Desenvolva uma HOF que retorna o resultado de um sorteio. Esta HOF irá gerar 
um número aleatório entre 1 e 5 recebendo como parâmetros o número apostado e uma
função que checa se o número apostado é igual ao número sorteado. O retorno da 
sua HOF deve ser uma string (Ex: "Tente novamente" ou "Parabéns você ganhou").*/

// Deselvolver uma Hof
// Retornar o resultado de um sorteio
// Gerar um número aleatório entre 1 e 5
// Parametro o número apostado
// Uma função que checa se o número apostado é igual ao número sorteado
// Retorno 'Tente novamente'  ou 'Parabéns, você ganhou'

const resultSorteio = (numero, random) => numero === random;

const check = (numero, callback) => {
  const random = Math.floor(Math.random() * 5) + 1;
  return callback(numero, random) ? "Parabéns, você ganhou" : "Tente novamente";
};
console.log(check(3, resultSorteio));
